...
function LandingPage() {
    ...
    restaurantArr.forEach((restaurant) => {
        let sum = 0
        restaurant.reviews ? restaurant.reviews.forEach((res) => {
            sum += res.stars
        }) : null
        ...
        <div>
            ...
            <div className="restaurantDivs">
                {restaurantArr?.map((restaurant) => (
                    <div className="restaurantCard" style={{ border: "2px solid black" }} key={restaurant.id} onClick={() => navigate(`restaurants/${restaurant.id}`)}>
                        <p>{restaurant.name}</p>
